...
class Charabase {
    ...
    updata() {
        ...
        if (this.x+(100<<8) < 0 || this.x-(100<<8) > FIELD_W << 8 || this.y+(100) < 0 || this.y-(100) > FIELD_H << 8) {
            this.kill = true;
        }
    }
    draw() {
...
class Star {
    constructor() {
        ...
        this.vx = 0;//横のベクトル
        this.vy = rand(30, 200);//縦のベクトル
        this.sz = rand(1, 2); //☆のサイズ
    }
    draw() {
        let x = this.x >> 8;
        let y = this.y >> 8;
